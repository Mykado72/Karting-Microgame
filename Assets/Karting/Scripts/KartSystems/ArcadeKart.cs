...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        public float BaseAccelerationPower = 15f;
        ...
        void GatherInputs()
        {
            ...
            for (int i = 0; i < m_Inputs.Length; i++)
            {
                ...
                WantsToDrift = Input.Jump; //; // Input.Brake && Vector3.Dot(Rigidbody.velocity, transform.forward) > 0.0f;
            }
        }
        ...
        void MoveVehicle(bool accelerate, bool brake, float turnInput)
        {
            ...
            float accelPower = accelDirectionIsFwd ? BaseAccelerationPower : BaseReverseAccelerationPower;
            ...
            // Appliquer le multiplicateur d'accélération (boost en sortie de drift)
            finalAccelPower *= m_CurrentAccelerationMultiplier;
            ...
            if (Mathf.Abs(accelInput) > k_NullInput)
            {
                ...
                // APRÈS: Utiliser la direction d'input au lieu de velocityDirection
                Vector3 accelDirection = Mathf.Sign(accelInput) > 0 ? transform.forward : -transform.forward;
                movement = accelDirection * Mathf.Abs(accelInput) * finalAcceleration *
            ...
            if (GroundPercent > 0.0f)  // au moins une roue est au sol, on peut tourner et gérer le drift
            {
                ...
                float actualAngularVelocitySteering = angularVelocitySteering;
                if (!localVelDirectionIsFwd && !accelDirectionIsFwd)
                    actualAngularVelocitySteering *= -1.0f;
                ...
                // ============================================================
                // GESTION DU DRIFT EXIT BOOST
                // ============================================================
                if (!IsDrifting && m_WasDriftingLastFrame)
                {
                    // On vient de sortir d'un drift!
                    Vector3 flattenVelocity = Vector3.ProjectOnPlane(Rigidbody.velocity, m_VerticalReference).normalized;
                    float alignmentBonus = Vector3.Dot(flattenVelocity, transform.forward);
                    alignmentBonus = Mathf.Clamp01(alignmentBonus);
                    
                    // Vérifier que la vitesse est suffisante
                    float speedPercent = currentSpeed / maxSpeed;
                    if (speedPercent >= DriftBoostMinSpeedPercent && alignmentBonus >= DriftBoostAlignmentThreshold)
                    {
                        // BOOST ACTIVÉ!
                        ...
                        #if UNITY_EDITOR
                        Debug.Log($"🚀 DRIFT BOOST ACTIVÉ! Alignment: {alignmentBonus:F2}, Speed: {speedPercent:F2}, Multiplier: {m_CurrentAccelerationMultiplier:F2}x");
                        #endif
                    }
                }

                // Gérer la durée du boost
                if (m_DriftBoostTimer > 0.0f)
                {
                    m_DriftBoostTimer -= Time.fixedDeltaTime;
                    
                    if (m_DriftBoostTimer <= 0.0f)
                    {
                        m_CurrentAccelerationMultiplier = 1.0f;
                        m_DriftBoostTimer = 0.0f;
                    }
                    else
                    {
                        float boostProgress = m_DriftBoostTimer / DriftExitBoostDuration;
                        m_CurrentAccelerationMultiplier = Mathf.Lerp(1.0f, 
                            1.0f + (DriftExitBoostMultiplier - 1.0f), 
                            boostProgress);
                    }
                }
                else
                ...
                m_WasDriftingLastFrame = IsDrifting;

                // Apply angular velocity
                angularVel.y = Mathf.MoveTowards(angularVel.y, turningPower * actualAngularVelocitySteering,Time.fixedDeltaTime * angularVelocitySmoothSpeed);
